...
multimorbidity_list = df.values.tolist()
...
for type in ['snp', 'gene', 'ppi', 'pathway', 'rg']:
    ...
    if type == 'rg':
        path1 = '../overlap/multimorbidity_rg.txt'
        path2 = '../overlap/permutation/rg.csv'

    multimorbidity_interpreted = set()
    df = pd.read_table(path1)
    list1 = df.values.tolist()
    for each in list1:
        multimorbidity_interpreted.add((each[0], each[1]))
    ...
    for each in multimorbidity_list:
        ...
        if code2 not in genetic_disease:
            continue
        if (code1, code2) in multimorbidity_interpreted:
