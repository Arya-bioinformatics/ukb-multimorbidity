...
for flag in ['with', 'without']:
    if flag == 'with':
        ...
        path6 = '../overlap/multimorbidity_snp.txt'
        path7 = '../overlap/multimorbidity_gene.txt'
        path8 = '../overlap/multimorbidity_ppi.txt'
        path9 = '../overlap/multimorbidity_pathway.txt'
        path10 = '../overlap/multimorbidity_rg.txt'
        path11 = '../overlap/multimorbidity_overlap.txt'
        ...
        path6 = '../overlap/multimorbidity_snp_rmhla.txt'
        path7 = '../overlap/multimorbidity_gene_rmhla.txt'
        path8 = '../overlap/multimorbidity_ppi_rmhla.txt'
        path9 = '../overlap/multimorbidity_pathway_rmhla.txt'
        path10 = '../overlap/multimorbidity_rg_rmhla.txt'
        path11 = '../overlap/multimorbidity_overlap_rmhla.txt'
    ...
    for each in list1:
        ...
        if (code1 in disease_snp) & (code2 in disease_snp):
            ...
            if len(set1) != 0:
                multimorbidity_snp[(code1, code2, des1, des2)] = set1
        if (code1 in disease_gene) & (code2 in disease_gene):
            set1 = disease_gene[code1] & disease_gene[code2]
            if len(set1) != 0:
                multimorbidity_gene[(code1, code2, des1, des2)] = set1
        if (code1 in disease_ppi) & (code2 in disease_ppi):
            ...
            for each1 in set3:
                ...
                if ((gene1 in set1) & (gene2 in set2)) | ((gene2 in set1) & (gene1 in set2)):
                    set4.add(each1)
            if len(set4) != 0:
                multimorbidity_ppi[(code1, code2, des1, des2)] = set4
        if (code1 in disease_pathway) & (code2 in disease_gene):
            ...
            for each1 in set1:
                ...
                if len(intersection) != 0:
                    set3.add(each1)
            if len(set3) != 0:
                if (code1, code2, des1, des2) not in multimorbidity_pathway:
                    multimorbidity_pathway[(code1, code2, des1, des2)] = set()
                multimorbidity_pathway[(code1, code2, des1, des2)] |= set3
        ...
        if (code1 + '~' + code2) in diseasePair_rg:
            ...
            multimorbidity_rg[(code1, code2, des1, des2)] = [rg, p]

    list1 = list()
    for each in multimorbidity_rg:
        list1.append([each[0], each[1], each[2], each[3], multimorbidity_rg[each][0], multimorbidity_rg[each][1]])
    ...
    multimorbidity_rg = dict()
    for each in list1:
        multimorbidity_rg[(each[0], each[1], each[2], each[3])] = [str(each[4]), str(each[5]), str(each[6])]

    with open(path6, 'w+') as outfile:
        outfile.write('code1\tcode2\tdescription1\tdescription2\tsnp\n')
        for each in multimorbidity_snp:
            outfile.write('\t'.join(each) + '\t' + ';'.join(multimorbidity_snp[each]) + '\n')
        outfile.close()
    ...
    with open(path7, 'w+') as outfile:
        ...
        for each in multimorbidity_gene:
            list1 = list(multimorbidity_gene[each])
            list1.sort()
            list2 = list()
            for each1 in list1:
    ...
    with open(path8, 'w+') as outfile:
        ...
        for each in multimorbidity_ppi:
            list1 = list(multimorbidity_ppi[each])
            list1.sort()
            list2 = list()
            for each1 in list1:
                gene1, gene2 = each1.split('~')
    ...
    with open(path9, 'w+') as outfile:
        ...
        for each in multimorbidity_pathway:
            outfile.write('\t'.join(each) + '\t' + ';'.join(multimorbidity_pathway[each]) + '\n')
        outfile.close()

    with open(path10, 'w+') as outfile:
        outfile.write('code1\tcode2\tdescription1\tdescription2\trg\tp\tq\n')
        for each in multimorbidity_rg:
            outfile.write('\t'.join(each) + '\t' + '\t'.join(multimorbidity_rg[each]) + '\n')
        outfile.close()
    ...
    print('genetic multimorbidity count: ' + str(len(genetic_multimorbidity)))