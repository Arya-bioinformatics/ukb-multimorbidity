...
multimorbidity_set = set()
df = pd.read_csv('../phenotype/multimorbidity_filter.csv')
list1 = df.values.tolist()
for each in list1:
    code1, code2 = each[:2]
    multimorbidity_set.add(code1 + '-' + code2)
...
for flag in ['with', 'without']:
    if flag == 'with':
        ...
        path6 = '../overlap/nonmultimorbidity_snp.txt'
        path7 = '../overlap/nonmultimorbidity_gene.txt'
        path8 = '../overlap/nonmultimorbidity_ppi.txt'
        path9 = '../overlap/nonmultimorbidity_pathway.txt'
        path10 = '../overlap/nonmultimorbidity_rg.txt'
        ...
        path6 = '../overlap/nonmultimorbidity_snp_rmhla.txt'
        path7 = '../overlap/nonmultimorbidity_gene_rmhla.txt'
        path8 = '../overlap/nonmultimorbidity_ppi_rmhla.txt'
        path9 = '../overlap/nonmultimorbidity_pathway_rmhla.txt'
        path10 = '../overlap/nonmultimorbidity_rg_rmhla.txt'
    ...
    nonmultimorbidity_snp = dict()
    nonmultimorbidity_gene = dict()
    nonmultimorbidity_ppi = dict()
    nonmultimorbidity_pathway = dict()
    nonmultimorbidity_rg = dict()
    for code1, code2 in combinations(genetic_disease, 2):
        if ((code1, code2) in multimorbidity_set) | ((code2, code1) in multimorbidity_set):
            continue
        if (code1 in disease_snp) & (code2 in disease_snp):
            set1 = disease_snp[code1] & disease_snp[code2]
            if len(set1) != 0:
                if code1 < code2:
                    nonmultimorbidity_snp[(code1, code2)] = set1
                else:
                    nonmultimorbidity_snp[(code2, code1)] = set1
        if (code1 in disease_gene) & (code2 in disease_gene):
            set1 = disease_gene[code1] & disease_gene[code2]
            if len(set1) != 0:
                if code1 < code2:
                    nonmultimorbidity_gene[(code1, code2)] = set1
                else:
                    nonmultimorbidity_gene[(code2, code1)] = set1
        if (code1 in disease_ppi) & (code2 in disease_ppi):
            ...
            if len(set4) != 0:
                if code1 < code2:
                    nonmultimorbidity_ppi[(code1, code2)] = set4
                else:
                    nonmultimorbidity_ppi[(code2, code1)] = set4
        if (code1 in disease_pathway) & (code2 in disease_gene):
            ...
            if len(set3) != 0:
                if code1 < code2:
                    if (code1, code2) not in nonmultimorbidity_pathway:
                        nonmultimorbidity_pathway[(code1, code2)] = set()
                    nonmultimorbidity_pathway[(code1, code2)] |= set3
                else:
                    if (code2, code1) not in nonmultimorbidity_pathway:
                        nonmultimorbidity_pathway[(code2, code1)] = set()
                    nonmultimorbidity_pathway[(code2, code1)] |= set3
        ...
        if (code1 + '~' + code2) in diseasePair_rg:
            ...
            nonmultimorbidity_rg[(code1, code2)] = [rg, p]
        elif (code2 + '~' + code1) in diseasePair_rg:
            [rg, p] = diseasePair_rg[code2 + '~' + code1]
            nonmultimorbidity_rg[(code2, code1)] = [rg, p]

    list1 = list()
    for each in nonmultimorbidity_rg:
        list1.append([each[0], each[1], nonmultimorbidity_rg[each][0], nonmultimorbidity_rg[each][1]])
    ...
    nonmultimorbidity_rg = dict()
    for each in list1:
        nonmultimorbidity_rg[(each[0], each[1])] = [str(each[2]), str(each[3]), str(each[4])]

    with open(path6, 'w+') as outfile:
        outfile.write('code1\tcode2\tsnp\n')
        for each in nonmultimorbidity_snp:
            outfile.write('\t'.join(each) + '\t' + ';'.join(nonmultimorbidity_snp[each]) + '\n')
        outfile.close()

    with open(path7, 'w+') as outfile:
        outfile.write('code1\tcode2\tgene id\tgene symbol\n')
        for each in nonmultimorbidity_gene:
            list1 = list(nonmultimorbidity_gene[each])
            list1.sort()
            list2 = list()
            for each1 in list1:
    ...
    with open(path8, 'w+') as outfile:
        ...
        for each in nonmultimorbidity_ppi:
            list1 = list(nonmultimorbidity_ppi[each])
            list1.sort()
            list2 = list()
            for each1 in list1:
                gene1, gene2 = each1.split('~')
    ...
    with open(path9, 'w+') as outfile:
        ...
        for each in nonmultimorbidity_pathway:
            outfile.write('\t'.join(each) + '\t' + ';'.join(nonmultimorbidity_pathway[each]) + '\n')
        outfile.close()

    with open(path10, 'w+') as outfile:
        outfile.write('code1\tcode2\trg\tp\tq\n')
        for each in nonmultimorbidity_rg:
            outfile.write('\t'.join(each) + '\t' + '\t'.join(nonmultimorbidity_rg[each]) + '\n')
        outfile.close()