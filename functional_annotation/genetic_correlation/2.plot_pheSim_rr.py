...
if __name__ == '__main__':
    ...
    multimorbidity_similarity = list()

    df = pd.read_csv('../EMR/phenotype/multimorbidity_filter.csv')
    ...
    for each in list1:
        ...
        if key1 in similarity_dict:
            ...
            continue
        multimorbidity_similarity.append([code1, code2, float(rr), float(s)])

    df = pd.DataFrame(multimorbidity_similarity, columns=['disease1', 'disease2', 'rr', 'PheSim'])
    df1 = df[df['rr']>60]
    print(df1.shape[0])
