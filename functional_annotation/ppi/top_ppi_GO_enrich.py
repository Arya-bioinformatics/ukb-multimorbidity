...
for flag in ['with', 'without']:
    print('------------\n' + flag + '\n------------')
    if flag == 'with':
        path1 = '../overlap/multimorbidity_ppi.txt'
        path2 = 'a.csv'
    if flag == 'without':
        path1 = '../overlap/multimorbidity_ppi_rmhla.txt'
        path2 = 'a1.csv'

    ppi_multimor_count = dict()
    ppi_multimorbidity = dict()
    ...
    with open(path1, 'r') as infile:
        for i, line in enumerate(infile):
            ...
            for each in set1:
                if each not in ppi_multimor_count:
                    ppi_multimor_count[each] = 0
                    ppi_multimorbidity[each] = set()
                ppi_multimor_count[each] += 1
                ppi_multimorbidity[each].add((code1, code2))
        infile.close()

    df = pd.DataFrame.from_dict(ppi_multimor_count, orient='index', columns=['count'])
    df['ppi'] = df.index
    ...
    print('all_ppi_interpreted multimorbiidty: ' + str(all_ppi_interpreted))
    print('top ppi involved multimorbidity: ' + str(len(set2)))
