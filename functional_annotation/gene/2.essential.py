...
for flag in ['with', 'without']:
    ...
    with open(path2, 'r') as infile:
        for i, line in enumerate(infile):
            ...
            str1 = line.strip('\r\n')
            str2 = str1.split('\t')
            multimorbidity_gene |= set(str2[4].split(';'))
        infile.close()

    print(' --------- gene count ---------')
    print('all gene count: ' + str(len(all_gene)))
    print('disease gene count: ' + str(len(disease_gene)))
    print('multimorbidity gene count: ' + str(len(multimorbidity_gene)))
    print('essential gene count: ' + str(len(essential_gene)))


    print('# ----- essential gene overlap ----- #')
    print(len(multimorbidity_gene & essential_gene) / float(len(multimorbidity_gene)))
    print(len(disease_gene & essential_gene) / float(len(disease_gene)))

    print(' --------- multimorbidity enrich ---------- ')
    a = len(multimorbidity_gene & essential_gene)
    b = len(essential_gene) - a
    c = len(multimorbidity_gene) - a
    d = len(all_gene) - a - b - c
    [odds, p] = fisher_exact([[a, b], [c, d]])
    print([odds, p])

    print('--------- multimorbidity enrich (backgroud disease) ----------')
    a = len(multimorbidity_gene & essential_gene)
    ...
    c = venn2([multimorbidity_gene, essential_gene], ('multimorbidity-genes', 'Essential genes'), alpha=1)
