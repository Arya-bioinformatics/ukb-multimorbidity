...
df = data.frame(Tissues=c('1', '2', '3', '4'), # 1:1 2:2-5 3:6-42 4:43-53
                ...
                Multimorbidity=as.numeric(unlist(data['Multimorbidity'])))
...
ggplot(melt(df), aes(x=variable, y=value, fill=Tissues)) + 
  ...
  geom_segment(data=df %>% arrange(by=desc(Tissues)) %>% mutate(GroupB=cumsum(Only_disease)) %>% mutate(GroupC=cumsum(Multimorbidity)), aes(x=2.25, xend=2.75, y=GroupB, yend=GroupC))+
