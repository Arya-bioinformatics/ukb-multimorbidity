...
for flag in ['with', 'without']:
    ...
    multimorbidity_gene_pLI = list()
    for each in gene_pLI:
        pLI = gene_pLI[each]
        if each in multimorbidity_gene:
            multimorbidity_gene_pLI.append(pLI)
        elif each in disease_gene:
            only_disease_gene_pLI.append(pLI)
        elif each in all_gene:
            nonassociated_gene_pLI.append(pLI)

    [u1, p1] = ttest_ind(multimorbidity_gene_pLI, nonassociated_gene_pLI)
    [u2, p2] = ttest_ind(multimorbidity_gene_pLI, only_disease_gene_pLI)
    ...
    for each in multimorbidity_gene_pLI:
        list1.append(['multimorbidity', each])
    ...
    b.set_xticklabels(['Non-disease genes', 'Other disease-genes', 'multimorbidity-genes'], rotation=45)
