...
for flag in ['with', 'without']:
    ...
    if flag == 'with':
        ...
        path2 = '../overlap/multimorbidity_gene.txt'
        path3 = 'a.csv'
        # continue
    if flag == 'without':
        path1 = '../genome/disease_gene_rmhla.txt'
        path2 = '../overlap/multimorbidity_gene_rmhla.txt'
        path3 = 'a1.csv'
        # continue
    ...
    with open(path2, 'r') as infile:
        for i, line in enumerate(infile):
            ...
            str1 = line.strip('\r\n')
            str2 = str1.split('\t')
            multimorbidity_gene = multimorbidity_gene | set(str2[4].split(';'))
        infile.close()

    nonassociated_gene_tissues = list()
    only_disease_gene_tissues = list()
    multimorbidity_gene_tissues = list()
    for each in all_gene_tissue_number:
        num = all_gene_tissue_number[each]
        if each in multimorbidity_gene:
            multimorbidity_gene_tissues.append(num)
        elif each in disease_gene:
            only_disease_gene_tissues.append(num)
        else:
            nonassociated_gene_tissues.append(num)

    [u1, p1] = mannwhitneyu(multimorbidity_gene_tissues, nonassociated_gene_tissues)
    [u2, p2] = mannwhitneyu(multimorbidity_gene_tissues, only_disease_gene_tissues)
    ...
    s3 = pd.Series(multimorbidity_gene_tissues)
    df = pd.DataFrame()
    df['Non-associated'] = s1
    df['Only disease'] = s2
    df['multimorbidity'] = s3

    r_list = [0, 1, 5, 42, 53]
    with open(path3, 'w+') as outfile:
        outfile.write('Tissue number,Non-associated,Only disease,multimorbidity\n')
        for i in range(0, len(r_list) - 1):
            ...
            df1 = df.loc[(df['multimorbidity'] > r_list[i]) & (df['multimorbidity'] <= r_list[i + 1])]
            c = df1.shape[0] / float(len(multimorbidity_gene_tissues))
            outfile.write(
