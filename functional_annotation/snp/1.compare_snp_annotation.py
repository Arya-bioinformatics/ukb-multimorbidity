...
for flag in ['with', 'without']:
    ...
    with open(path1, 'r') as infile:
        for i, line in enumerate(infile):
            ...
            str1 = line.strip('\r\n')
            str2 = str1.split('\t')
            multimorbidity_snp |= set(str2[-1].split(';'))
        infile.close()
    ...
    multimorbidity_type = dict()
    count1 = 0
    count2 = 0
    count3 = 0
    for each in annovar_gene_annotation:
        type = annovar_gene_annotation[each]
        if each in multimorbidity_snp:
            if type not in multimorbidity_type:
                multimorbidity_type[type] = 0
            multimorbidity_type[type] += 1
            count1 += 1
    ...
    for each in all_type:
        ...
        c = multimorbidity_type[each]
        list1.append([each, a, b, c, float(a) / count3, float(b) / count2, float(c) / count1])

    df = pd.DataFrame(list1, columns=['annotation', 'nonassociated', 'only_disease', 'multimorbidity',
                               'Ratio_of_nonassociated', 'Ratio_of_only_disease', 'Ratio_of_multimorbidity'])
    df.to_csv(path5, index=False)

    # enrichment
    print('------------ compared to all -------------')
    for each in all_type:
        a = multimorbidity_type[each]
        b = nonassciated_type[each] + only_disease_type[each]
        c = len(multimorbidity_snp) - a
        d = len(all_snp) - a - b - c
    ...
    [t, p] = ttest_ind(multimorbidity_score, nonassociated_score)
    print([t, p])
    print('------------ compare to only disease --------------')
    [t, p] = ttest_ind(multimorbidity_score, only_disease_score)
    print([t, p])
    ...
    for each in multimorbidity_score:
        list1.append(['multimorbidity-SNPs', each])
    ...
    b.set_xticklabels(['Non-disease SNPs', 'Other disease-SNPs', 'multimorbidity-SNPs'], rotation=45)
    ...
    [t, p] = ttest_ind(multimorbidity_score, nonassociated_score)
    print([t, p])
    print([np.mean(multimorbidity_score), np.mean(nonassociated_score)])
    print('------------ compare to only disease --------------')
    [t, p] = ttest_ind(multimorbidity_score, only_disease_score)
    print([t, p])
    print([np.mean(multimorbidity_score), np.mean(only_disease_score)])
    print('------------ compare disease to nonassociated --------------')
    [t, p] = ttest_ind(only_disease_score, nonassociated_score)
    print([t, p])
    ...
    for each in only_disease_score:
        list1.append(['Other disease-SNPs', each])
    for each in multimorbidity_score:
        list1.append(['multimorbid-SNPs', each])
    ...
    b.tick_params(labelsize=16)
    b.set_xticklabels(['Non-disease SNPs', 'Other disease-SNPs', 'multimorbid-SNPs'], rotation=45)
    plt.savefig(path6, bbox_inches='tight')
    plt.close()