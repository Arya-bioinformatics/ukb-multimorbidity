...
df = data.frame(Region=c('Noncoding RNA', 'Intergenic region', 'Genic region'),
                ...
                Multimorbidity=as.numeric(unlist(data['Ratio_of_Multimorbidity'])))
...
ggplot(melt(df), aes(x=variable, y=value, fill=Region)) + 
  ...
  geom_segment(data=df %>% arrange(by=desc(Region)) %>% mutate(GroupB=cumsum(Only_disease)) %>% mutate(GroupC=cumsum(Multimorbidity)), aes(x=2.25, xend=2.75, y=GroupB, yend=GroupC))+
