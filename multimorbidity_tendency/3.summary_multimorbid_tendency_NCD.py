...
multimor_df = pd.read_csv('../phenotype/multimorbidity_filter.csv', dtype=str)
all_multimor_disease = set(multimor_df['disease1']) | set(multimor_df['disease2'])
...
for each in list1:
    ...
    if icd9 in ICD9_ICD10_map:
        ...
        if icd10 in all_multimor_disease:
...
df = pd.read_csv('../network_random/multimorbid_tendency_NCD.csv')
list1 = df.values.tolist()
...
for each in list1[1:]:
    ...
    if s >= original:
        count += 1
print('inner multimorbid tendency as a whole: ' + str(float(count)/df.shape[0]))


list1 = list(df.iloc[0])
category_category_order = list(df.columns)
category_category_multimorbidity_original = dict()
for i in range(0, len(list1)):
    category_category_multimorbidity_original[category_category_order[i]] = list1[i]

category_category_multimorbidity_significance = dict()
for each in category_category_order:
    count = df[df[each]>=df[each][0]].shape[0]
    category_category_multimorbidity_significance[each] = float(count)/df.shape[0]

list1 = list()
for each in category_category_multimorbidity_significance:
    c1, c2 = each.split('-')
    set1 = category_disease[c1]
    set2 = category_disease[c2]
    count = category_category_multimorbidity_original[each]
    if c1 == c2:
    ...
    significance = category_category_multimorbidity_significance[each]
    list1.append([c1, c2, count, ratio, significance])

df = pd.DataFrame(list1, columns=['category1', 'category2', 'multimorbidity count', 'Jaccard index', 'p'])
df['q'] = multitest.fdrcorrection(df['p'])[1]
df.to_csv('../network_random/multimorbidity_category_significance_NCD.txt', sep='\t', index=False)
