...
multimorbidity_eat_drug_frequency = dict()
df = pd.read_csv('../overlap/drug/multimorbidity_drug_score.csv')
...
for each in list1:
    ...
    drug = each[2]
    frequency = each[3]
    multimorbidity_eat_drug_frequency[(code1, code2, drug)] = frequency


result = list()
multimorbidity_drug_overlap = dict()
...
for each in list1:
    ...
    for each1 in ukb_name:
        if (code1, code2, each1) in multimorbidity_eat_drug_frequency:
            frequency = multimorbidity_eat_drug_frequency[(code1, code2, each1)]
