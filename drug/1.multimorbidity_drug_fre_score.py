...
if __name__ == '__main__':
    ...
    df = pd.read_csv('/../phenotype/multimorbidity_filter.csv', dtype=str)
    df1 = df[['disease1', 'disease2']]
    multimorbidity_list = df1.values.tolist()

    list1 = list()
    for each in multimorbidity_list:
        code1 = each[0]
        code2 = each[1]
    ...
    multimorbidity_drug_score = list()
    i = 0
    for each in multimorbidity_list:
        print(i)
        i = i + 1
        ...
        for each1 in drug_set:
            ...
            x1 = len(a)
            x2 = len(b)
            multimorbidity_drug_score.append([code1, code2, name, str(float(x1) / x2)])
    df2 = pd.DataFrame(multimorbidity_drug_score, columns=['disease1', 'disease2', 'drug', 'frequency'])
    df2.to_csv('/../overlap/drug/multimorbidity_drug_score.csv', index=False)