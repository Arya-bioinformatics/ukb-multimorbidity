...
df = pd.read_table('../overlap/multimorbidity_snp.txt')
list1 = df.values.tolist()
for each in list1:
    multimorbidity_set.add((each[0], each[1]))
df = pd.read_table('../overlap/multimorbidity_gene.txt')
list1 = df.values.tolist()
for each in list1:
    multimorbidity_set.add((each[0], each[1]))
    multimorbidity_gene[(each[0], each[1])] = set(each[5].split(';'))
...
for each in LG_module_disease:
    ...
    for each1 in hub_disease:
        ...
        for each2 in disease_multimorbidity[each1]:
            if each2 not in multimorbidity_gene:
                continue
            gene_list += list(multimorbidity_gene[each2])
        list1 = list()
...
df = pd.DataFrame(list1, columns=['multimorbidity Module', 'Module size', 'Disease', 'Featured categories', 'Hub diseases', 'Top 5 genes shared by hub diseases'])
