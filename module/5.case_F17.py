...
module_multimorbidity = set(multimorbidity_gene.keys()) & diseasePair_set

list1 = list()
for each in module_multimorbidity:
    if 'F17' in each:
        print([disease_name[each[0]], disease_name[each[1]], disease_class[each[0]], disease_class[each[1]]])

dict1 = dict()
for each in multimorbidity_gene:
    if ('F17' in each) & (each in module_multimorbidity):
        for each1 in multimorbidity_gene[each]:
...
for each in multimorbidity_gene:
    if ('F17' in each) & (len(set(each) & cluster_disease[F17_cluster]) == 2):
        cytoscape_input[each] = multimorbidity_gene[each]
