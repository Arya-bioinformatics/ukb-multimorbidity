...
df = pd.read_table('../overlap/multimorbidity_ppi.txt')
list1 = df.values.tolist()
for each in list1:
    multimorbidity_set.add((each[0], each[1]))
    multimorbidity_ppi[(each[0], each[1])] = set(each[5].split(';'))
...
for each in NG_module_disease:
    ...
    module_multimorbidity = module_diseasePair & multimorbidity_set
    disease_multimorbidity = dict()
    for each1 in set1:
        set2 = set()
        for each2 in module_multimorbidity:
            if each1 in each2:
                set2.add(each2)
        disease_multimorbidity[each1] = set2
    list1 = list()
    for each1 in disease_multimorbidity:
        list1.append([each1, len(disease_multimorbidity[each1])])
    df = pd.DataFrame(list1, columns=['disease', 'multimorbidity number'])
    df = df.sort_values(by='multimorbidity number', ascending=False)
    list1 = list(set(df['multimorbidity number']))
    ...
    for each1 in df.values.tolist():
        ...
        if number in top_3_num[:1]:
            ...
            set2 |= disease_multimorbidity[disease]
    if len(set2) / len(module_multimorbidity) < 0.5:
        hub_disease = list()
        set2 = set()
        for each1 in df.values.tolist():
            disease, number = each1
    ...
    for each1 in hub_disease:
        ppi_list = list()
        for each2 in disease_multimorbidity[each1]:
            if each2 not in multimorbidity_ppi:
                continue
            ppi_list += list(multimorbidity_ppi[each2])
        list1 = list()
        ...
        pathway_list = list()
        for each2 in disease_multimorbidity[each1]:
            if each2 not in multimorbidity_pathway:
                continue
            pathway_list += list(multimorbidity_pathway[each2])
        list1 = list()
...
df = pd.DataFrame(list1, columns=['multimorbidity Module', 'Module size', 'Disease', 'Featured categories', 'Hub diseases', 'Top 5 PPI shared by hub diseases', 'Top 5 Pathway shared by hub diseases'])
