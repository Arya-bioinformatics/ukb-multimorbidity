...
if __name__ == '__main__':
    ...
    list1 = df.values.tolist()
    disease_multimor_count = dict()
    for each in list1:
        code1 = each[0]
        code2 = each[1]
        if code1 not in disease_multimor_count:
            disease_multimor_count[code1] = 0
        if code2 not in disease_multimor_count:
            disease_multimor_count[code2] = 0
        disease_multimor_count[code1] += 1
        disease_multimor_count[code2] += 1

    df = pd.DataFrame.from_dict(disease_multimor_count, orient='index', columns=['count'])
    df = df.sort_values(by='count', ascending=False)
    df.to_csv('a.csv')

    list1 = list()
    for each in disease_multimor_count:
        count = disease_multimor_count[each]
    ...
    print('disease with more than 100 multimorbidities: ' + str(i) + '\n')
    ...
    plt.xlabel('Number of multimorbidities (log10)', fontsize=15)
    ...
    plt.ylabel('multimorbidity count', fontsize=16)
