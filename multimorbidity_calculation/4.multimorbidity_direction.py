...
for each in list1:
    ...
    multimor_person = disease_patient[code1] & disease_patient[code2]
    Na = 0
    Nb = 0
    double_patients = double_disease_patient[code1 + '-' + code2]
    Nsame = len(double_patients)
    for each1 in multimor_person:
...
df.to_csv('../phenotype/multimorbidity_filter_direction.csv', index=False)