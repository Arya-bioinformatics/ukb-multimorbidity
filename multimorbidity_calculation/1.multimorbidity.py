...
with open('../phenotype/multimorbidity.csv', 'w+') as multimorbidityFile:
    multimorbidityFile.write('disease1' + ',' + 'disease2' + ',' + 'patient number of disease1' + ',' + 'patient number of disease2' + ',' + 'patiant number of disease1 and disease2' + ','
                              + 'RR' + ',' + 'p_value' + '\n')
    multimorbidityFile.close()
...
print('all multimorbidity combination: ' + str(len(all_combination)))
result = list()
for each in all_combination:
    ...
    multimorbidity_patient = patient_set1 & patient_set2
    Ii = len(patient_set1)
    Ij = len(patient_set2)
    Cij = len(multimorbidity_patient)
    rr = RR(Ii, Ij, Cij, N)
    ...
    if len(result) == 1000:
        with open('../phenotype/multimorbidity.csv', 'a') as multimorbidityFile:
            multimorbidityFile.write('\n'.join(result) + '\n')
        multimorbidityFile.close()
        result = list()
